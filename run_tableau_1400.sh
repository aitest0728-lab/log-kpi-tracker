...
#
# v28.0: index.html and dashboard.html merged into one file (index.html now
# does live-fetch-with-embedded-fallback itself, see kpi_pipeline.py's
# update_embedded_data()). kpi_pipeline.py now rewrites public/index.html's
# embedded-data block on every run, so it needs to be staged and pushed like
# the JSON files - added here. dashboard.html is no longer generated at all;
# if an old copy is still tracked in the repo, `git rm public/dashboard.html`
# it once by hand so it stops shipping a stale, increasingly-out-of-date
# snapshot alongside the merged index.html.
